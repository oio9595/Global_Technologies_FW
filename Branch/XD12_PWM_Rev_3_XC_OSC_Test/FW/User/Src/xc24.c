...
void XC24_Get_Trim_Parameter(float* p_ldo, float* p_osc, float* p_dac)
{
    ...
    *p_ldo = (float)(ADC_VOLT_PER_STEP * ext_adc_value) / CONST_mV_TO_V; // Dac out convert to V
    ...
    *p_osc = JigBD_IF_Get_Input_Capture_Freq() * XC24_CONST_FREQ_DIVIDE / CONST_MHz_TO_Hz;

    // Get DAC 1V
    XC24_Trim_Init_DAC_Gain();
    ADS114S08_Select_Input_CH(ADS114S08_CH_XC_DAC);
    XC24_Write_Register(XC24_ADDR_CURRENT_TARGET_DAC, 1241);
    LL_mDelay(1);
    ADS114S08_Set_Start(1);
    ADS114S08_Wait_Done();
    ext_adc_value = ADS114S08_Get_ADC_Value();
    *p_dac = (float)(ADC_VOLT_PER_STEP * ext_adc_value) / CONST_mV_TO_V;
}
