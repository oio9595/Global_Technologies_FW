...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
    ...
    XC_Trim_IF_Set_OTP_Enable(TRUE);
...
/* ----------------- command list - ui ----------------- */
        ...
        else if (Command_is_("5"))
        {
            ...
            float trim_ldo = 0.0f;
            float trim_osc = 0.0f;
            float trim_dac_1v = 0.0f;

            float now_ldo = 0.0f;
            float now_osc = 0.0f;
            float now_dac_1v = 0.0f;

            XC24_Get_Trim_Parameter(&trim_ldo, &trim_osc, &trim_dac_1v);

            print(LOG_INFO, "\r\n XC Trim Parameter LDO : %.3f , OSC : %.3f , DAC_1V : %.3f \r\n", trim_ldo, trim_osc, trim_dac_1v);

            XC24_Write_Register(XC24_ADDR_SOFT_RESET, 0x0007);
            us_delay(SPI_DELAY); //reset delay ???
#if 1
            for (uint8_t osc_val = 0x7F/*0x40*/ ; osc_val < 0x80 ; ++osc_val)
            {
                for (uint8_t trial = 0 ; trial < 100 ; ++trial)
                {
                    uint16_t f2_val = ((osc_val << 8) | 0x08);
                    XC24_Set_OTP_Protect(false);
                    us_delay(SPI_DELAY);
                    XC24_Write_Register(XC24_MIRROR_ADDR_MIRROR1, f2_val);
                    us_delay(SPI_DELAY);
                    XC24_Write_Register(XC24_MIRROR_ADDR_OTP_RD_PROG, 2);
                    us_delay(SPI_DELAY);

                    XC24_Get_Trim_Parameter(&now_ldo, &now_osc, &now_dac_1v);

                    print(LOG_INFO, "Trial  : OTP_F2 : 0x%02X , Trial : %3d , LDO : %.3f , OSC : %.3f , DAC_1V : %.3f", \
                        osc_val, trial, now_ldo, now_osc, now_dac_1v);
                    if ((fabs(now_osc - trim_osc) <= 0.01f) && (fabs(now_ldo - trim_ldo) <= 0.01f) && (fabs(now_dac_1v - trim_dac_1v) <= 0.03f))
                    {
                        print(LOG_INFO, "  OK \r\n");
                    }
                    else
            ...
            for (uint8_t osc_val = 0x40 ; osc_val < 0x80 ; ++osc_val)
            {
                print(LOG_INFO, "%u\r\n", error[osc_val - 0x40]);
            }
#endif
