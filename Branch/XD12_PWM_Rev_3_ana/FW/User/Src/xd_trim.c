...
// 4mA
//#define COMP_A    (9708)
//#define COMP_B    (320000)

// 8mA
//#define COMP_A    (10000)
//#define COMP_B    (326000)

// 12mA
#define COMP_A      (10100)
#define COMP_B      (330000)
#define CURR_MIN    (0.74f)
#define CURR_MAX    (8.53f)

// 16mA
//#define COMP_A    (10330)
//#define COMP_B    (361000)
...
// 32mA
//#define COMP_A      (10300)
//#define COMP_B      (360000)
//#define CURR_MIN    (2.04f)
//#define CURR_MAX    (22.43f)
/* ---------------------------------------------------------------------------------------------------------------------- */
...
static current_gain_t gt_screen_gain;
    ...
    static uint32_t gn_xd_origin_vref;
    static uint32_t gn_xd_convert_vref;
#else
...
{
    int temp_vref = 0;
    uint16_t calc_vref = 0;
    float ideal_iout = 0.0f;
    if (gn_task_delay)
    {
        --gn_task_delay;
    }
...
// Point
        ...
        case XD_SCREEN_STEP_CHANGE_OUTPUT :
            #if (XD_SCREEN_TYPE == XD_SCREEN_ANA)
                ideal_iout = ((CURR_MAX - CURR_MIN) * gn_xd_origin_vref / 1023.0f) + CURR_MIN;
// Point
                calc_vref = (ideal_iout * 4095 / 12.0f);
                temp_vref = calc_vref * COMP_A - COMP_B;
                if (temp_vref < 0)
                {
                    temp_vref = 0;
                }
                else if (temp_vref > 4095 * 10000)
                {
                    temp_vref = 4095 * 10000;
                }
                gn_xd_convert_vref = (uint16_t)((temp_vref) / 10000.0f + 0.5f);
                XDIC_Set_Max_Curr_Vref(gn_xd_convert_vref);
            #else
        ...
        case XD_SCREEN_STEP_GET_ADC_CH :
            ...
            else
            {
                ...
                #if (XD_SCREEN_TYPE == XD_SCREEN_ANA)
// Point
                    ideal_iout = 12.0f * ((float)gn_xd_origin_vref / 4095.0f);
                    print(LOG_INFO, "%4u, %4u, %.5f, %.5f, %.5f, %.5f, %.5f, %.5f, %.5f, %.5f, %.5f, %.5f, %.5f, %.5f, %.5f\r\n", gn_xd_origin_vref, gn_xd_convert_vref, ideal_iout, \
                    ...
                    if (gn_xd_origin_vref == 1023)
                    {
                        gt_xd_screen_step = XD_SCREEN_STEP_STOP;
                    }
                    else
                    {
                        gn_xd_origin_vref += 20;
                        //gn_xd_origin_vref += 10;
                        if (gn_xd_origin_vref > 1023)
                        {
                            gn_xd_origin_vref = 1023;
                        }
                        gt_xd_screen_step = XD_SCREEN_STEP_CHANGE_OUTPUT;
                    }
                #else
