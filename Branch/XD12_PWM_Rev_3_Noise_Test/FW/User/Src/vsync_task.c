...
volatile bool kkk = true;

void XDIC_Vsync_Task(void)
{
    static uint8_t vsync_task_tick = 0;
    if (gb_xdic_vsync_flag)
    {
        ...
        {
            XDIC_Write_General_Reg(XDIC_ADDR_MAX_CURRENT_VREF, 500);
            XDIC_Write_General_Reg(XDIC_ADDR_VREF_FIX, 0xFFF);
        }
#if 0
        if (vsync_task_tick < 100)
        {
        }
        ++vsync_task_tick;
        if (vsync_task_tick >= 100)
