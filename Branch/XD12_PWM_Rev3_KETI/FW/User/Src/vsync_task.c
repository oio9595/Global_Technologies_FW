...
void XDIC_Vsync_Task(void)
{
    if (gb_xdic_vsync_flag)
    {
        ...
        if (gb_xd_max_curr_lvl_set)
        {
            XDIC_Set_Max_Current_Level((dev_max_curr_level_t)gn_xd_max_curr_lvl);
            us_delay(100);
            gb_xd_max_curr_lvl_set = false;
        }
        if (gb_xd_vrev_set)
        {
            XDIC_Set_Max_Curr_Vref(gn_xd_vref);
            us_delay(100);
            gb_xd_vrev_set = false;
        }
