...
static void XDIC_Set_Register_Type(uint8_t reg_type)
{
    ...
    {
        print(LOG_PC, "ERROR: %s - Invalid Register Type !!\r\n", __func__);
        return;
    }
...
#else
    XDIC_Write_General_Reg(XDIC_ADDR_MAX_CURRENT_VREF, 0xA5A);
    XDIC_Write_General_Reg(XDIC_ADDR_MAX_CURRENT_VREF, 0xA5A);
    XDIC_Write_General_Reg(XDIC_ADDR_MAX_CURRENT_VREF, 0xA5A);
#endif
    XC24_IF_IdGen_Command();
...
void XDIC_Init_VREF(void)
{
    ...
    XDIC_Write_Mirror_Reg(XDIC_ADDR_OTP_PROTECT, XDIC_OTP_PROTECT_DISABLE);
#if 0
    for (uint8_t i = 0 ; i < 100 ; ++i)
    {
        XDIC_Write_Mirror_Reg(XDIC_MIRROR_ADDR_VREF_CTL, 63); // VREF Control
        LL_mDelay(5);
        XDIC_Write_Mirror_Reg(XDIC_MIRROR_ADDR_VREF_CTL, 32); // VREF Control
        LL_mDelay(5);
    }
    XDIC_Write_Mirror_Reg(XDIC_ADDR_OTP_RD_PROG, 2);
#endif
    XDIC_Write_Mirror_Reg(XDIC_MIRROR_ADDR_VREF_CTL, 0); // VREF Control
}
...
uint16_t XDIC_Read_Mirror_Reg(uint8_t addr)
{
    ...
    if (map)
    {
        ...
        xdic_reg_val = XC24_IF_Read_XDIC(addr);
        *((uint16_t*)(map->reg_ptr)) = xdic_reg_val;
        print(LOG_PC, "XDIC Mirror Read --> [ 0x%02X - 0x%04X] \r\n", addr, xdic_reg_val);
    }
    else
    {
        print(LOG_PC, "ERROR: %s - addr(0x%02X) Not Found !!\r\n", __func__, addr);
