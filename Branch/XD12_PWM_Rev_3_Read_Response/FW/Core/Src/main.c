...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define DELAY_NOT_INTENDED      (3)
#define DELAY_INTENDED          (1000)
#define DELAY_nINT_TO_OUT_US    (DELAY_INTENDED - DELAY_NOT_INTENDED)
...
volatile bool gb_xd_read_response_flag;
...
int main(void)
{
  ...
  while (1)
  {
    ...
    if (gb_xd_read_response_flag)
    {
        us_delay(DELAY_nINT_TO_OUT_US);
        JigBD_IF_Read_Respose_Command();
        gb_xd_read_response_flag = false;
    }
  }
  /* USER CODE END 3 */
}
...
static void MX_GPIO_Init(void)
{
  ...
  /**/
  LL_GPIO_SetPinPull(DEBUG_GPIO_Port, DEBUG_Pin, LL_GPIO_PULL_UP);

  /**/
  LL_GPIO_SetPinMode(ADC_DRDY_GPIO_Port, ADC_DRDY_Pin, LL_GPIO_MODE_INPUT);

  /**/
  LL_GPIO_SetPinMode(DEBUG_GPIO_Port, DEBUG_Pin, LL_GPIO_MODE_INPUT);
