...
void XC24_Init(void)
{
    ...
    for (uint8_t xc_addr = XC24_ADDR_SOFT_RESET ; xc_addr < XC24_ADDR_MAX ; ++xc_addr)
    {
        ...
        if (map)
        {
            ...
            case XC24_ADDR_CLK_CONTROL_1:
                ...
                gt_xc24_general_regs._r1B.serializer_skew_en = 1;
                gt_xc24_general_regs._r1B.osc_spread_en = 1;
                gt_xc24_general_regs._r1B.spread_dir = 2;
                gt_xc24_general_regs._r1B.spread_speed = 3;
                gt_xc24_general_regs._r1B.spread_range_a = 7;
                gt_xc24_general_regs._r1B.spread_range_d = 3;
#endif
                break;
    ...
    //while (1)
    {
        ...
        uint16_t temp_1[2] = {0x8001, 0x0001};
        SPI_Write(g_hSPIx, temp_1, 2);

        uint16_t temp_2[2] = {0xBCC1, 0x0002};
        SPI_Write(g_hSPIx, temp_2, 2);

        uint16_t temp_3[2] = {0x8001, 0x0030};
        SPI_Write(g_hSPIx, temp_3, 2);

        uint16_t temp_4[2] = {0x8201, 0x0111};
        SPI_Write(g_hSPIx, temp_4, 2);

        uint16_t temp_5[2] = {0x8341, 0x0601};
        SPI_Write(g_hSPIx, temp_5, 2);

        uint16_t temp_6[2] = {0x8441, 0x0101};
        SPI_Write(g_hSPIx, temp_6, 2);

        uint16_t temp_7[4] = {0x84C3, 0x0101, 0x0810, 0x016E};
        SPI_Write(g_hSPIx, temp_7, 4);

        uint16_t temp_8[2] = {0x86C1, 0x5206};
        SPI_Write(g_hSPIx, temp_8, 2);

        uint16_t temp_9[3] = {0x87C2, 0x2C4B, 0x000A};
        SPI_Write(g_hSPIx, temp_9, 3);

        uint16_t temp_10[21] = {0x8C14, 0x2528, 0x2128, 0x0109, 0x0000, 0x0109, \
                                0x0000, 0x2128, 0x2509, 0x5A50, 0x505A, 0x505A, \
                                0x505A, 0x0000, 0x0000, 0x505A, 0x0000, 0x0000, \
                                0x5A50, 0x5A50, 0x5A50};
        SPI_Write(g_hSPIx, temp_10, 21);

        uint16_t temp_11[3] = {0x9142, 0x30FF, 0xD8FC};
        SPI_Write(g_hSPIx, temp_11, 3);

        uint16_t temp_12[2] = {0x9341, 0x0101};
        SPI_Write(g_hSPIx, temp_12, 2);

        uint16_t temp_13[2] = {0x93C1, 0x0FA0};
        SPI_Write(g_hSPIx, temp_13, 2);

        uint16_t temp_14[4] = {0x9643, 0x0578, 0x0180, 0x0FA0};
        SPI_Write(g_hSPIx, temp_14, 4);

        for (uint8_t i = 0 ; i < 100 ; ++i)
        {
            uint16_t temp[2] = {0x8101, 0x8000};
            SPI_Write(g_hSPIx, temp, 2);
            us_delay(1010);
        }
    ...
    uint16_t temp_15[2] = {0x8441, 0x0101};
    SPI_Write(g_hSPIx, temp_15, 2);

    uint16_t temp_16[2] = {0x9C01, 0xFFFF};
    SPI_Write(g_hSPIx, temp_16, 2);

    uint16_t temp_17[2] = {0x9C41, 0xFFFF};
    SPI_Write(g_hSPIx, temp_17, 2);

    uint16_t temp_18[2] = {0x9C81, 0xFFFF};
    SPI_Write(g_hSPIx, temp_18, 2);

    uint16_t temp_19[2] = {0x9CC1, 0xFFFF};
    SPI_Write(g_hSPIx, temp_19, 2);

    uint16_t temp_20[2] = {0x9D01, 0xFFFF};
    SPI_Write(g_hSPIx, temp_20, 2);

    uint16_t temp_21[2] = {0x9D41, 0xFFFF};
    SPI_Write(g_hSPIx, temp_21, 2);

    uint16_t temp_22[2] = {0x9D81, 0xFFFF};
    SPI_Write(g_hSPIx, temp_22, 2);

    uint16_t temp_23[2] = {0x9DC1, 0xFFFF};
    SPI_Write(g_hSPIx, temp_23, 2);

    uint16_t temp_24[2] = {0x80C1, 0x8000};
    SPI_Write(g_hSPIx, temp_24, 2);

    uint16_t temp_25[9] = {0x5C08, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000};
    SPI_Write(g_hSPIx, temp_25, 9);
#endif
}
...
{
    gt_xc24_mirror_regs._rF6.dac_ofs_sign = ((dac_ofs & 0x80) >> 7);
	gt_xc24_mirror_regs._rF6.dac_ofs_val = ((dac_ofs & 0x7F) >> 0);
