...
static screen_param_t gt_xd_screen_param[XDIC_ICTL_SCREEN_POINT];

void XD_Screen_Init(void)
{
    gt_xd_screen_param[0].f_target_min = XDIC_ICTL_L_SCREEN_TARGET1 * (1.0f - XDIC_ICTL_SCREEN_ERR_RATE);
    gt_xd_screen_param[0].f_target_max = XDIC_ICTL_L_SCREEN_TARGET1 * (1.0f + XDIC_ICTL_SCREEN_ERR_RATE);
    gt_xd_screen_param[0].vref_point = XDIC_ICTL_L_SCREEN_P1;
    gt_xd_screen_param[0].gain = GAIN_MID;

    gt_xd_screen_param[1].f_target_min = XDIC_ICTL_L_SCREEN_TARGET2 * (1.0f - XDIC_ICTL_SCREEN_ERR_RATE);
    gt_xd_screen_param[1].f_target_max = XDIC_ICTL_L_SCREEN_TARGET2 * (1.0f + XDIC_ICTL_SCREEN_ERR_RATE);
...
void XD_Trim_Calculate_Spec(void)
{
    for (xd_trim_mode_t mode = XD_TRIM_START ; mode < XD_TRIM_MAX ; ++mode)
    {
        ...
        case XD_TRIM_OSC_FREQUENCY:
            ...
            break;
        case XD_TRIM_ICTL_L_CHS:
            gt_xdic_trim_condition[mode].f_target_min = (XDIC_ICTL_L_TARGET * (1 - XDIC_ICTL_L_ERR_RATE)) / 1000;
            gt_xdic_trim_condition[mode].f_target_max = (XDIC_ICTL_L_TARGET * (1 + XDIC_ICTL_L_ERR_RATE)) / 1000;
            gt_xdic_trim_condition[mode].u16_p1 = XDIC_ICTL_L_P1;
            gt_xdic_trim_condition[mode].u16_p2 = XDIC_ICTL_L_P2;
            break;
        case XD_TRIM_ICTL_H_CHS:
            gt_xdic_trim_condition[mode].f_target_min = (XDIC_ICTL_H_TARGET * (1 - XDIC_ICTL_H_ERR_RATE)) / 1000;
            gt_xdic_trim_condition[mode].f_target_max = (XDIC_ICTL_H_TARGET * (1 + XDIC_ICTL_H_ERR_RATE)) / 1000;
            gt_xdic_trim_condition[mode].u16_p1 = XDIC_ICTL_H_P1;
            gt_xdic_trim_condition[mode].u16_p2 = XDIC_ICTL_H_P2;
            break;
        }
    }
}
        ...
        else
        {
            ...
            case XD_TRIM_STEP_CHECK:
                ...
                case XD_TRIM_OSC_FREQUENCY:
                    ...
                    u8_tmp_channel_max = 1;
                    t_trim_search_mode_next = XD_TRIM_ICTL_L_CHS;
            ...
            case XD_TRIM_STEP_SCREEN:
                ...
                for (uint8_t i = 0 ; i < XDIC_ICTL_SCREEN_POINT ; ++i)
                {
                    ...
                    float target_avg = (gt_xd_screen_param[i].f_target_min + gt_xd_screen_param[i].f_target_max) / 2.0f;
                    gt_xd_screen_param[i].f_error = ((gt_xd_screen_param[i].f_avg - target_avg) / target_avg) * 100.0f; // Calculate error percentage
                    print(LOG_INFO, "%u, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f\r\n",
                        gt_xd_screen_param[i].judge_flag, gt_xd_screen_param[i].f_error, gt_xd_screen_param[i].f_avg,
