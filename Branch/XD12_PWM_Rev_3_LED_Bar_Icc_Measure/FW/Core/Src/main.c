...
void btn_process(void)
{
    ...
    now_state = LL_GPIO_IsInputPinSet(B1_GPIO_Port, B1_Pin);
...
#define ADC_SAMPLE_NUM 2000 // about 0.5ms per sample number
...
uint16_t adc_value[ADC_SAMPLE_NUM] = {0, };
float icc_value[ADC_SAMPLE_NUM] = {0, };

static void icc_measure(void)
{
    for (uint16_t i = 0 ; i < ADC_SAMPLE_NUM ; ++i)
    {
        ...
        adc_value[i] = ADS114S08_Get_ADC_Value();
        DEBUG_LO();
    }

    for (uint16_t i = 0 ; i < ADC_SAMPLE_NUM ; ++i)
    {
        icc_value[i] = JigBD_IF_Convert_Adc_To_Current(adc_value[i], GAIN_HIGH);
    }
...
static void TaskDebugUart(void)
{
    ...
    if (comm_get_rx_packet(&p_data))
    {
        ...
        else if (Command_is_("icc_start") || Command_is_("1"))
        {
            icc_measure();
        }
