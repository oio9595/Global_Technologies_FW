...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
    ...
    /* for GUI App */
    print(LOG_INFO, "XD12 BFI IS SELECTED!\r\n");
...
/* ----------------- command list - xd ----------------- */
        ...
        else if(Command_Param_is_("xd_test_en", "%d", &u32_recv_param[0]))
        {
            if (u32_recv_param[0])
            {
                TargetIC_IF_Set_Test_Enable(TRUE);
            }
            else
            {
                TargetIC_IF_Set_Test_Enable(FALSE);
            }
        }
        ...
        else if(Command_Param_is_("xd_sf_bfi", "%d", &u32_recv_param[0]))
        {
            XD12_set_sf_bfi((uint16_t)u32_recv_param[0]);

            print(LOG_INFO, "\r\n xd12_sf_bfi - [%u]\r\n", u32_recv_param[0]);
        }
        else if(Command_is_("xd_sf_bfi"))
        {
            print(LOG_INFO, "\r\n xd12_sf_bfi - [%u]\r\n", TargetIC_IF_Read_Register(XD12_ADDR_SF_BFI_SIZE, XD12_REG_TYPE_NON_TRIM));
        }
        ...
        {
            XD12_set_ld_fix((u32_recv_param[0] << 11));
            print(LOG_INFO, "\r\n LD_FIX to %u", u32_recv_param[0]);
        }
