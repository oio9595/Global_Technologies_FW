...
static bool gb_xdic_run_test_task;
...
{
    if (gb_xdic_run_test_task == false)
    {
        gb_xdic_run_test_task = true;
    }
    if (gn_xd_rx_timeout)
    {
        --gn_xd_rx_timeout;
...
void comm_print_help(void)
{
    ...
    print(LOG_INFO, "\n\r  jig_ch_sel [d]\t : Select output channel (0 ~ XDIC_CH_MAX)");
    ...
    print(LOG_INFO, "\n\r  xd_ch [d]\t\t : Set XDIC channel (0 ~ XDIC_CH_MAX)");
...
static void JigTestMainTask(void)
{
    if (gb_xdic_run_test_task)
    {
        XDIC_Test_Task();
        gb_xdic_run_test_task = false;
    }
}
...
/* ----------------- command list - jig ----------------- */
        ...
        {
            if (u32_recv_param[0] > (gn_xdic_channel_size + 1))
            {
                print(LOG_ERROR, "\r\n Out of CH SEL [%d] [0 - %d]\r\n", u32_recv_param[0], gn_xdic_channel_size);
            }
            else
            {
                print(LOG_INFO, "\r\n XD CH SEL - [%d]\r\n", u32_recv_param[0]);
...
/* ----------------- command list - ui ----------------- */
        else if (Command_is_("xd_test_start") || Command_is_("1"))
        {
            print(LOG_INFO, "\r\n XD Trim Start \r\n");
            XDIC_Test_Start();
        }
