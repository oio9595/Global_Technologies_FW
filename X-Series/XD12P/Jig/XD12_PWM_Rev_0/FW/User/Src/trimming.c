...
static uint8_t trimming_algorithm_body(_trim_algo_param *ptr_Param)
{
    ...
    if(channel < u8_CH_MAX)
    {
        ...
        u16_reg_value_cur = TargetIC_IF_TrimRegister_Get(channel, ptr_Param->trim_mode);
    ...
    if(ptr_Param->u8_sTrimSaved_Cnt >= TRIM_REGISTER_SAVED_CNT)
    {
        ...
        else
        {
            ...
            TargetIC_IF_TrimRegister_Set(channel, ptr_Param->trim_mode, ptr_Param->sTrimSaved[u8_closest_adc_index].u16_saved_reg);
...
/* TRIMMING_STEP_CHANGE_OUTPUT  ***************************************/
        case TRIMMING_STEP_CHANGE_OUTPUT:
            ...
            case TRIM_ICTL_L_CHS:
            case TRIM_ICTL_H_CHS:
                if(gt_trim_search_mode == TRIM_ICTL_L_CHS)
                {
                    TargetIC_IF_TRIM_Init_ICTL_L();
                }
                else //TRIM_ICTL_H_CHS
                {
                ...
                if(gn_slope_cnt == 0)
                {
                    XD12_set_max_curr_level((dev_max_curr_level_t)glf_TrimPara_GUI[gt_trim_search_mode][TRIM_PARA_P1]);
                }
                else
                {
                    XD12_set_max_curr_level((dev_max_curr_level_t)glf_TrimPara_GUI[gt_trim_search_mode][TRIM_PARA_P2]);
                }
...
void screening_procedure_run(void)
{
    ...
    case SCREEN_STEP_CHANGE_OUTPUT :
        ...
        #else
            XD12_set_max_curr_level(gt_xd_screen_max_curr_level);
        #endif
