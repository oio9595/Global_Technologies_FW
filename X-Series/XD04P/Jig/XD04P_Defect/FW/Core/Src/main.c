...
void comm_print_help(void)
{
    ...
    print(LOG_INFO, "\n\r  \"xd_ldim [d]\"\t\t\t : Set LDIM value [0 ~ 16383]");
    ...
    print(LOG_INFO, "\n\r  \"xd_dimming_start\" or \"3\"\t : Start XDIC Vsync Dimming Process");
    print(LOG_INFO, "\n\r  \"xd_ec_start\" or \"5\"\t\t : Start XDIC Electrical Characteristic Measure Process");
    print(LOG_INFO, "\n\r  \"xd_ch_start\" or \"6\"\t\t : Start XDIC Neighbor Channel Effect Measure Process");

    print(LOG_INFO, "\n\r  \"xd_line_delay [d]\"\t\t : [0] : Disable Line Delay / [1] : Enable Line Delay");
    print(LOG_INFO, "\n\r  \"xd_scan_no [d]\"\t\t : Set XDIC SCAN_NO [0 ~ 5]");;
...
/* ----------------- command list - jig ----------------- */
        else if (Command_is_("test1"))
        {
            XDIC_Test_1();
        }
        else if (Command_is_("test2"))
        {
            XDIC_Test_2();
        }
...
/* ----------------- command list - xd ----------------- */
        ...
        else if (Command_Param_is_("xd_ldim_sweep", "%d", &u32_recv_param[0]))
        {
            if (u32_recv_param[0])
            {
        ...
        else if (Command_Param_is_("xd_ldim", "%d", &u32_recv_param[0]))
        {
            ...
            {
                print(LOG_INFO, "\r -> Set ldim to [%u (%.3f[%%])]\r\n\r\n", u32_recv_param[0], ((float)u32_recv_param[0] * 100.0f) / 16383.0f);
        ...
        {
            uint16_t ld_data = XDIC_Get_LD_Data();
            print(LOG_INFO, "\r\n ldim - [%u (%.3f[%%])]\r\n", ld_data , ((float)XDIC_Get_LD_Data() * 100.0f) / 16383.0f);
        }
        ...
        else if (Command_is_("xd_trim_ictl_h"))
        {
            ...
            for (uint8_t i = 0 ; i < XD_CH_SIZE ; ++i)
            {
                ...
                ADS114S08_Wait_Done();
                iout_adc[i][0] = ADS114S08_Get_ADC_Value();
//                print(LOG_INFO, "\r\n ICTL_H CH[%d]_First : %u\r\n", i, iout_adc[i][0]);
                XDIC_Set_Max_Curr_Vref(1000);
                ADS114S08_Set_Start(1);
                ADS114S08_Wait_Done();
                iout_adc[i][1] = ADS114S08_Get_ADC_Value();
//                print(LOG_INFO, "\r\n ICTL_H CH[%d]_Second : %u\r\n", i, iout_adc[i][1]);
                uint16_t adc_average = (iout_adc[i][0] + iout_adc[i][1]) / 2;
                float iout_avg = JigBD_IF_Convert_Adc_To_Current(adc_average, current_gain);
                print(LOG_INFO, "\r\n ICTL_H CH[%d] : %.3f\r\n", i, iout_avg);
            }
        ...
        else if (Command_Param_is_("xd_tail", "%d", &u32_recv_param[0]))
        {
            uint16_t reg1_value;
            reg1_value = XDIC_Read_General_Reg(0x01);
            uint16_t bit_mask = 1;
            reg1_value &= (~bit_mask);
            if(0 == u32_recv_param[0])
            {
                XDIC_Write_General_Reg(0x01, reg1_value);
                print(LOG_INFO, "\r\nHead Mode Set\r\n");
            }
            else
            {
