...
TickType_t g_CHK_TRIM_TIME_TRIM = 0;
...
static uint8_t gn_current_gain = GAIN_HIGH;
...
int amp_gain_test = 0;
static int Current_OFS_DIR_FLAG;
...
extern uint32_t gn_ads114s08_adc_temp;
extern uint32_t gn_ads114s08_adc_accu_count;
extern uint16_t gn_adc_read_count;

trimming_step_t gt_jig_trimming_step;
trimming_step_t gt_jig_trimming_step_previous;
...
static uint8_t gn_e2p_program[5]= { 0x20, 0x03, 0x00, 0xA5, 0x10 }; /* E2P program byte sequense for OTP write */

static uint8_t gn_read_adc_vout_channel = 5;
static uint16_t gn_trim_adc_reault[TRIM_MAX][APIC_OUTPUT_NUM];
...
static uint8_t gn_aqic_trim_ofs_mached_cnt = 0;
static uint8_t gn_aqic_trim_ictl_mached_cnt = 0;
static uint16_t gn_trim_temp_cnt[TRIM_MAX];
static uint8_t gn_trim_ch_cnt = 0;
static uint8_t gn_trim_gain_level_ch_a = 0;

static uint32_t gn_adc_average;
static uint8_t gn_trim_ofs_step[APIC_OUTPUT_NUM];
static uint8_t gn_trim_ictl_step[APIC_OUTPUT_NUM];

static min_max_t gt_trimming_output_measure[TRIM_MAX];

static uint16_t gn_step_delay;

static uint8_t gb_found_max_level;
static uint8_t gn_found_current_slope_step;
static double gd_current_slope[APIC_OUTPUT_NUM];
static uint16_t gn_current_slope_adc[2][APIC_OUTPUT_NUM];

static uint16_t average_output_measure_max;
static uint16_t average_output_measure_min;
...
static double gn_aqic_offset[APIC_OUTPUT_NUM];
...
extern void set_amic_power(uint8_t on);
extern  void set_amic_mode(uint8_t en);
...
extern void aqic_set_mode(uint8_t mode);
...
extern void us_delay(uint32_t n_delay)
{
	...
	while(htim4.Instance->CNT < n_delay);
}

void apic_test_mode_enter(void)
{
    change_i2c_setting(I2C_USED_I2C);  /* change gpio to i2c */
	gn_current_gain = GAIN_HIGH;
    change_current_gain(gn_current_gain);

    set_amic_mode(GEN_PWM); /* pwm duty ~50%, freq 1MHz */

	aqic_mode_select(MODE_DAC);
	dac_set_voltage(3.5f, AQIC_MODE);

    HAL_Delay(5);

    set_amic_power(PWR_ON); /* APIC power on */
...
void aqic_mode_select(uint8_t sel)
{
	if (sel == MODE_BF)
	{
		// HAL_GPIO_WritePin(MODE_SEL_GPIO_Port, MODE_SEL_Pin, GPIO_PIN_RESET);
        LATCH__WritePin(MODE_SEL_LS_CP, MODE_SEL_LS_PORT, LATCH_PIN_RESET);
		dac_set_voltage(5.0f, AQIC_MODE);  /* D=0.0V */

	}

	else if (sel == MODE_DAC)
	{
		// HAL_GPIO_WritePin(MODE_SEL_GPIO_Port, MODE_SEL_Pin, GPIO_PIN_SET);
...
static uint8_t gn_ictl_lsb_test_count;
...
void trimming_procedure_run(void)
{
    ...
    uint8_t temp = 0;
    uint8_t channel = 0;
    uint8_t temp_i2c[8] = {0, };
    uint8_t sub_addr = 0;
    uint16_t u16_pre_register[APIC_OUTPUT_NUM] = {0,};
    uint16_t u16_pre_adc[APIC_OUTPUT_NUM] = {0,};
    ...
    switch(gt_jig_trimming_step)
    {
        case TRIMMING_STEP_INIT:
            ...
            gb_found_max_level = 0;
            gn_ictl_lsb_test_count = 0;
            Current_OFS_DIR_FLAG = 0;
            gn_trim_ch_cnt = 0;

            gn_found_current_slope_step = 0;
            gd_current_slope[0] = 0.0;
            gd_current_slope[1] = 0.0;
            gd_current_slope[2] = 0.0;
            gd_current_slope[3] = 0.0;
            gd_current_slope[4] = 0.0;
            gd_current_slope[5] = 0.0;
            ...
            for(uint16_t item = 0 ; item < TRIM_MAX ; ++item)
            {
                for(uint16_t ch = 0 ; ch < APIC_OUTPUT_NUM ; ++ch)
                {
                    gn_trim_adc_reault[item][ch] = 0;
                }
            }
        ...
        case TRIMMING_STEP_APIC_PWR:
#ifdef CHK_TRIM_TIME
            g_CHK_TRIM_TIME_tick = xTaskGetTickCount();
            ...
            aqic_mode_select(MODE_DAC);
            HAL_Delay(10);
            aqic_set_mode(test_mode);

            aqic_mode_select(MODE_BF);
            set_amic_mode(GEN_PWM); /* pwm duty ~50%, freq 1MHz */
            change_i2c_setting(I2C_USED_I2C);  /* change gpio to i2c */
        ...
        case TRIMMING_STEP_EXT_CLK_OFF:
            #ifdef CHK_TRIM_TIME
            g_CHK_TRIM_TIME_INIT = xTaskGetTickCount() - g_CHK_TRIM_TIME_tick;
            ...
            if(gb_GUI_ACTIVATED)
            {
                ...
                gt_jig_trimming_step_previous = TRIMMING_STEP_EXT_CLK_OFF;
                break;
            }
            else
            {
        ...
        case TRIMMING_STEP_READ_REGISTER:
            ...
            else
            {
                ...
                for (uint8_t ch = 0; ch < AQ06_O_MAX ; ch++)
                {
        ...
        case TRIMMING_STEP_CHANGE_OUTPUT_INIT:
            ...
            if(gt_trim_search_mode == TRIM_SCREENING && gn_screen_cnt == 0)
            {
                ...
                print("jig:scrn:");
                gt_aqic_reg_trim_e0.u.czmax1;
                gt_aqic_reg_trim_e1.u.czmax2;
            ...
            HAL_I2C_Mem_Write(&hi2c1, I2C_APIC_ID, I2C_SUB_TEST1, 1, &(gt_aqic_reg_test1.value), 1, 10);
            us_delay(10);    /* TBD */
            aqic_output_select(gn_read_adc_vout_channel);
            /*
            GAIN_LOW :  1mA
            GAIN_MID :  5mA
            GAIN_HIGH : 50mA
            */
            switch(gt_trim_search_mode)
            {
                case TRIM_OFS_LIN_CHS:
                    gn_current_gain = GAIN_LOW;
                    change_current_gain(gn_current_gain);
                    current_discharge(CHARGE);
                    break;

                case TRIM_ICTL_CHS:
                    gn_current_gain = gn_trim_gain_level_ch_a;
                    change_current_gain(gn_current_gain);
                    current_discharge(CHARGE);
                    // snprintf(msg_buffer, sizeof msg_buffer, "\r\nTRIM_ICTL_CHS - GAIN_[%d]\r\n",gn_current_gain);
                    // print(msg_buffer);
                    break;
        ...
        case TRIMMING_STEP_CHANGE_OUTPUT:
            ...
            {
                case TRIM_OFS_LIN_CHS:
                    dac_set_voltage(gf_trim_voltage_table[gt_trim_search_mode][gn_aqic_slope_cnt], AQIC_D_ALL);
                    us_delay(10);
                ...
                case TRIM_OUTPUT_TEST:
                    ...
                    dac_set_voltage(gf_trim_voltage_table[gt_trim_search_mode][0], AQIC_D_ALL);  /* current */
                    ...
                    dac_set_voltage(gf_trim_voltage_table[gt_trim_search_mode][1], AQIC_D_ALL);  /* pwm */
                    us_delay(3);    /* TBD, Time for dac voltage output to stabilize */
                    break;
                ...
                case TRIM_APIC_OUTPUT_TEST:
                    ...
                    {
                        dac_set_voltage(gf_apic_output_test_table[gn_apic_output_test_index], AQIC_D_ALL);
                    }
                    else
                    {
                        dac_set_voltage(1.000f, AQIC_D_ALL);
                    }
                    us_delay(3);    /* TBD, Time for dac voltage output to stabilize */
                    gt_aqic_reg_test1.u.sel_l_ch = (0x01U << gn_read_adc_vout_channel);
                    HAL_I2C_Mem_Write(&hi2c1, I2C_APIC_ID, I2C_SUB_TEST1, 1, &(gt_aqic_reg_test1.value), 1, 10);
                    ...
                    if(gn_apic_output_mode == 0)
                    {
                        dac_set_voltage(5.000f, AQIC_D_ALL);
                    }
                    else
                    {
                        dac_set_voltage(gf_apic_output_test_table[gn_apic_output_test_index], AQIC_D_ALL);
				...
				case TRIM_SCREENING:
                    ...
                    {
                        dac_set_voltage(0.5, AQIC_D_ALL);
                    }
                    else
                    {
                        dac_set_voltage(gf_trim_voltage_table[gt_trim_search_mode][0] + gn_screen_cnt * SCREEN_STEP, AQIC_D_ALL);
                    }
        ...
        case TRIMMING_STEP_GET_ADC_CH:
            if(gb_ads114s08_drdy_done == 1)
            {
                ...
                {
                    gn_aqic_slope_adc[gn_aqic_slope_cnt][gn_read_adc_vout_channel] = (uint16_t)(gn_ads114s08_adc_temp/gn_ads114s08_adc_accu_count);
                    gn_aqic_slope_cnt++;
                    if (gn_aqic_slope_cnt >= 2)
                    {
                        gn_aqic_slope_cnt = 0;
                        gt_trimming_output_measure[gt_trim_search_mode].min = 0xFFFF;
                        gt_trimming_output_measure[gt_trim_search_mode].max = 0;
                ...
                {
                    gn_aqic_screen_adc[gn_read_adc_vout_channel] = (uint16_t)(gn_ads114s08_adc_temp/gn_ads114s08_adc_accu_count);
                ...
                {
                    gn_trim_adc_reault[gt_trim_search_mode][gn_read_adc_vout_channel] = (uint16_t)(gn_ads114s08_adc_temp/gn_ads114s08_adc_accu_count);
                    ++gn_read_adc_vout_channel;
                    if(gn_read_adc_vout_channel >= APIC_OUTPUT_NUM)
                    {
                        gn_read_adc_vout_channel = 0;
            ...
            switch(gt_trim_search_mode)
            {
                ...
                case TRIM_OFS_LIN_CHS:
                    ...
                    if(gn_read_adc_vout_channel < APIC_OUTPUT_NUM)
                    {
                        ...
                        {
                            case AQ06_O1 :
                                gn_aqic_slope_adc[1][channel] -= gn_adc_compensate[gn_current_gain][AQ06_O1];
                                gn_aqic_slope_adc[0][channel] -= gn_adc_compensate[gn_current_gain][AQ06_O1];
                                break;

                            case AQ06_O2 :
                                gn_aqic_slope_adc[1][channel] -= gn_adc_compensate[gn_current_gain][AQ06_O2];
                                gn_aqic_slope_adc[0][channel] -= gn_adc_compensate[gn_current_gain][AQ06_O2];
                                break;

                            case AQ06_O3 :
                                gn_aqic_slope_adc[1][channel] -= gn_adc_compensate[gn_current_gain][AQ06_O3];
                                gn_aqic_slope_adc[0][channel] -= gn_adc_compensate[gn_current_gain][AQ06_O3];
                                break;

                            case AQ06_O4 :
                                gn_aqic_slope_adc[1][channel] -= gn_adc_compensate[gn_current_gain][AQ06_O4];
                                gn_aqic_slope_adc[0][channel] -= gn_adc_compensate[gn_current_gain][AQ06_O4];
                                break;

                            case AQ06_O5 :
                                gn_aqic_slope_adc[1][channel] -= gn_adc_compensate[gn_current_gain][AQ06_O5];
                                gn_aqic_slope_adc[0][channel] -= gn_adc_compensate[gn_current_gain][AQ06_O5];
                                break;

                            case AQ06_O6 :
                                gn_aqic_slope_adc[1][channel] -= gn_adc_compensate[gn_current_gain][AQ06_O6];
                                gn_aqic_slope_adc[0][channel] -= gn_adc_compensate[gn_current_gain][AQ06_O6];
                                break;
                        }
                        ...
                        gn_aqic_slope[channel] = 1000 * convert_adc_to_current(channel, gn_aqic_slope_adc_temp[channel], gn_current_gain);
                        ...
                        if(gn_aqic_slope_adc_temp[channel] < temp_adjust_trim_range_min)
                        {
                            ...
                            // Check Addional Margin is matched.
                            if(gn_aqic_slope_adc_temp[channel] > (temp_adjust_trim_range_min - temp_adjust_trim_range_half))
                            {
                                gn_trim_ofs_regi_saved[(gn_aqic_trim_ofs_mached_cnt % TRIM_REGISTER_SAVED_CNT)].u16_saved_regi = ofs_regi_value;
                                gn_trim_ofs_regi_saved[(gn_aqic_trim_ofs_mached_cnt % TRIM_REGISTER_SAVED_CNT)].u16_saved_adc = gn_aqic_slope_adc_temp[channel];
                                gn_aqic_trim_ofs_mached_cnt++;
                                #ifdef DBG_TRIM_ALGORITHM
                    ...
                    // Check Vivration
                    if(gn_aqic_trim_ofs_mached_cnt >= TRIM_REGISTER_SAVED_CNT)
                    {
                        uint16_t ui_tartget_adc = (temp_adjust_trim_range_max + temp_adjust_trim_range_max)/2;
                        ...
                        {
                            ui_adc_gap_temp = abs(gn_trim_ofs_regi_saved[i].u16_saved_adc - ui_tartget_adc);
                    ...
                    snprintf(msg_buffer, sizeof msg_buffer, "  %d    %7u/%7u %7u   %7.3f   %7.3f   %7.3f   %7u  %7s\r\n",
                        ...
                        gn_aqic_slope_adc_temp[channel], gn_aqic_slope[channel], convert_adc_to_current(channel, gn_aqic_slope_adc[0][channel], gn_current_gain), convert_adc_to_current(channel, gn_aqic_slope_adc[1][channel], gn_current_gain), ofs_regi_value,
                    ...
                    if(temp == 0)
                    {
                        gn_trim_temp_cnt[gt_trim_search_mode]++;
                    }
                    else
                    {
                        gn_trim_temp_cnt[gt_trim_search_mode] = 0;
                    }

                    if(gn_read_adc_vout_channel >= APIC_OUTPUT_NUM)
                    {
                        #ifdef CHK_TRIM_TIME
                        g_CHK_TRIM_TIME_TRIM = xTaskGetTickCount() - g_CHK_TRIM_TIME_tick;
                ...
                case TRIM_ICTL_CHS:
                    ...
                    if(gn_read_adc_vout_channel < APIC_OUTPUT_NUM)
                    {
                        ...
                        gn_aqic_slope[channel] = convert_adc_to_current(channel, gn_aqic_slope_adc_temp[channel], gn_current_gain);
                        if (gn_aqic_slope_adc_temp[channel] < temp_adjust_trim_range_min)
                        {
                            temp |= (1<<channel);
                            gn_trim_adjust_flag[gt_trim_search_mode][channel] = ADJ_PLUS;
                            ...
                            if(gn_aqic_slope_adc_temp[channel] > (temp_adjust_trim_range_min - temp_adjust_trim_range_half))
                            {
                                gn_trim_ictl_regi_saved[(gn_aqic_trim_ictl_mached_cnt % TRIM_REGISTER_SAVED_CNT)].u8_saved_regi = ictl_regi_value;
                                gn_trim_ictl_regi_saved[(gn_aqic_trim_ictl_mached_cnt % TRIM_REGISTER_SAVED_CNT)].u16_saved_adc = gn_aqic_slope_adc_temp[channel];
                                gn_aqic_trim_ictl_mached_cnt++;
                                #ifdef DBG_TRIM_ALGORITHM
                        ...
                        else if(gn_aqic_slope_adc_temp[channel] > temp_adjust_trim_range_max)
                        {
                            ...
                            if(gn_aqic_slope_adc_temp[channel] < (temp_adjust_trim_range_max + temp_adjust_trim_range_half))
                            {
                                ...
                                gn_aqic_trim_ictl_mached_cnt++;
                                #ifdef DBG_TRIM_ALGORITHM
                                snprintf(msg_buffer, sizeof msg_buffer, "gn_aqic_trim_ictl_mached_cnt[%d]\r\n",gn_aqic_trim_ictl_mached_cnt);
                                print(msg_buffer);
                                #endif
                            }
                        }
                        else
                        {
                            ...
                            for(int i=0;i < TRIM_REGISTER_SAVED_CNT; i++)
                            {
                                ...
                                {
                                    gn_read_adc_vout_channel++;
                                    gn_aqic_trim_ictl_mached_cnt = 0;
                    ...
                    // Check Vivration
                    if(gn_aqic_trim_ictl_mached_cnt >= TRIM_REGISTER_SAVED_CNT)
                    {
                        uint16_t ui_tartget_adc = (temp_adjust_trim_range_max + temp_adjust_trim_range_max)/2;
                        uint16_t ui_adc_gap_closest = 0xffff;
                        uint16_t ui_adc_gap_temp = 0;
                        ...
                        for(uint8_t i=0; i < TRIM_REGISTER_SAVED_CNT; i++ )
                        {
                            ui_adc_gap_temp = abs(gn_trim_ictl_regi_saved[i].u16_saved_adc - ui_tartget_adc);
                            if( ui_adc_gap_temp < ui_adc_gap_closest )
                            {
                                ui_adc_gap_closest = ui_adc_gap_temp;
                                ub_closest_adc_index = i;
                ...
                case TRIM_SCREENING:
                    temp = 0;
                    for (uint8_t channel = 0 ; channel < AQ06_O_MAX ; channel++)
                    {
                        switch (channel)
                        {
                            case AQ06_O1 :
                                gn_aqic_screen_adc[channel] -= gn_adc_compensate[gn_current_gain][AQ06_O1];
                                break;

                            case AQ06_O2 :
                                gn_aqic_screen_adc[channel] -= gn_adc_compensate[gn_current_gain][AQ06_O2];
                                break;

                            case AQ06_O3 :
                                gn_aqic_screen_adc[channel] -= gn_adc_compensate[gn_current_gain][AQ06_O3];
                                break;

                            case AQ06_O4 :
                                gn_aqic_screen_adc[channel] -= gn_adc_compensate[gn_current_gain][AQ06_O4];
                                break;

                            case AQ06_O5 :
                                gn_aqic_screen_adc[channel] -= gn_adc_compensate[gn_current_gain][AQ06_O5];
                                break;

                            case AQ06_O6 :
                                gn_aqic_screen_adc[channel] -= gn_adc_compensate[gn_current_gain][AQ06_O6];
                                break;
                        }
                    }
                    for (uint8_t ch = 0 ; ch < APIC_OUTPUT_NUM ; ch++)
                    {
                        screen_cur[ch] = convert_adc_to_current(0, gn_aqic_screen_adc[ch], gn_current_gain);
                ...
                case TRIM_OUTPUT_TEST:
                    snprintf(msg_buffer, sizeof msg_buffer, "\t TRIM_OUTPUT_TEST : %5u, %5u, %5u, %5u, %5u, %5u\r\n", gn_trim_adc_reault[gt_trim_search_mode][0],gn_trim_adc_reault[gt_trim_search_mode][1],gn_trim_adc_reault[gt_trim_search_mode][2],gn_trim_adc_reault[gt_trim_search_mode][3],gn_trim_adc_reault[gt_trim_search_mode][4],gn_trim_adc_reault[gt_trim_search_mode][5]);
                    print(msg_buffer);
                    snprintf(msg_buffer, sizeof msg_buffer, "\t              %7.3f, %7.3f, %7.3f, %7.3f, %7.3f, %7.3f [mV]\r\n", convert_adc_to_voltage(0, gn_trim_adc_reault[gt_trim_search_mode][0]),convert_adc_to_voltage(1, gn_trim_adc_reault[gt_trim_search_mode][1]),convert_adc_to_voltage(2, gn_trim_adc_reault[gt_trim_search_mode][2]),convert_adc_to_voltage(3, gn_trim_adc_reault[gt_trim_search_mode][3]),convert_adc_to_voltage(4, gn_trim_adc_reault[gt_trim_search_mode][4]),convert_adc_to_voltage(5, gn_trim_adc_reault[gt_trim_search_mode][5]));
                    print(msg_buffer);
                    snprintf(msg_buffer, sizeof msg_buffer, "\t              %7.3f, %7.3f, %7.3f, %7.3f, %7.3f, %7.3f [mA]\r\n", convert_adc_to_current(0, gn_trim_adc_reault[gt_trim_search_mode][0], gn_current_gain),convert_adc_to_current(1, gn_trim_adc_reault[gt_trim_search_mode][1], gn_current_gain),convert_adc_to_current(2, gn_trim_adc_reault[gt_trim_search_mode][2], gn_current_gain),convert_adc_to_current(3, gn_trim_adc_reault[gt_trim_search_mode][3], gn_current_gain),convert_adc_to_current(4, gn_trim_adc_reault[gt_trim_search_mode][4], gn_current_gain),convert_adc_to_current(5, gn_trim_adc_reault[gt_trim_search_mode][5], gn_current_gain));
                    print(msg_buffer);
                ...
                case TRIM_APIC_OUTPUT_TEST:
                    //snprintf(msg_buffer, sizeof msg_buffer, "Di = %5.4f[V]\r\n", gf_apic_output_test_table[gn_apic_output_test_index]);
                    //print(msg_buffer);
                    //snprintf(msg_buffer, sizeof msg_buffer, "%5u, %5u, %5u, %5u, %5u, %5u\r\n", gn_trim_adc_reault[gt_trim_search_mode][0],gn_trim_adc_reault[gt_trim_search_mode][1],gn_trim_adc_reault[gt_trim_search_mode][2],gn_trim_adc_reault[gt_trim_search_mode][3],gn_trim_adc_reault[gt_trim_search_mode][4],gn_trim_adc_reault[gt_trim_search_mode][5]);
                    //print(msg_buffer);
                    //snprintf(msg_buffer, sizeof msg_buffer, "%9.6f, %9.6f, %9.6f, %9.6f, %9.6f, %9.6f [mV]\r\n", convert_adc_to_voltage(0, gn_trim_adc_reault[gt_trim_search_mode][0]),convert_adc_to_voltage(1, gn_trim_adc_reault[gt_trim_search_mode][1]),convert_adc_to_voltage(2, gn_trim_adc_reault[gt_trim_search_mode][2]),convert_adc_to_voltage(3, gn_trim_adc_reault[gt_trim_search_mode][3]),convert_adc_to_voltage(4, gn_trim_adc_reault[gt_trim_search_mode][4]),convert_adc_to_voltage(5, gn_trim_adc_reault[gt_trim_search_mode][5]));
                    //print(msg_buffer);
                    //snprintf(msg_buffer, sizeof msg_buffer, "%9.6f, %9.6f, %9.6f, %9.6f, %9.6f, %9.6f [mA]\r\n", convert_adc_to_current(0, gn_trim_adc_reault[gt_trim_search_mode][0]),convert_adc_to_current(1, gn_trim_adc_reault[gt_trim_search_mode][1]),convert_adc_to_current(2, gn_trim_adc_reault[gt_trim_search_mode][2]),convert_adc_to_current(3, gn_trim_adc_reault[gt_trim_search_mode][3]),convert_adc_to_current(4, gn_trim_adc_reault[gt_trim_search_mode][4]),convert_adc_to_current(5, gn_trim_adc_reault[gt_trim_search_mode][5]));
...
#ifdef CHK_TRIM_TIME
                g_CHK_TRIM_TIME_OTP = xTaskGetTickCount() - g_CHK_TRIM_TIME_tick;
        ...
        case TRIMMING_STEP_RESULT:
            if(gn_trim_out_of_spec_stop_trimming == 1)
            {
                ...
                PlayBeep(KEY_TEST_NG_BEEP, RESULT_BEEP_TIME);
            ...
            else
            {
                ...
                PlayBeep(KEY_TEST_OK_BEEP, RESULT_BEEP_TIME);
...
uint8_t read_reg_I2C(uint16_t in_addr)
{
    uint8_t sub_addr = 0;
    uint8_t dummy[8] = {0, };
    ...
    snprintf(msg_buffer, sizeof msg_buffer, "======== in_addr reg_I2C:%02x ========\r\n", in_addr);
    print(msg_buffer);
    ...
    if(HAL_OK == HAL_I2C_Master_Transmit(&hi2c1, I2C_APIC_ID, &sub_addr, 1, 1))
    {
        if(HAL_OK == HAL_I2C_Master_Receive(&hi2c1, I2C_APIC_ID, dummy, 1, 1))
        {
            snprintf(msg_buffer, sizeof msg_buffer, "read_reg_I2C I2C_SUB_DEVICEID :  %3u\r\n", dummy[0]);
            print(msg_buffer);
        }
        else
        {
...
void GUI_trim_RangeTbl(uint16_t in_trim_rang[])
{
