...
typedef void (*pJump_Function)(void);
typedef struct vector
{
    uint32_t        stack_addr; /* intvec[0] is initial Stack Pointer */
    pJump_Function  func_p;     /* intvec[1] is initial Program Counter */
}vector_t;
...
static uint8_t boot_img_jump(uint32_t app_abs_addr_start);
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
    ...
    do{
        ...
        /* Launch to Application Image */
        b_launch = boot_img_jump(APP_IMAGE_BEGIN);
    }while(b_launch == 0U);
...
static void MX_GPIO_Init(void)
{
  ...
  /**/
  GPIO_InitStruct.Pin = LL_GPIO_PIN_2|LL_GPIO_PIN_12|LL_GPIO_PIN_14|LL_GPIO_PIN_3
...
{
    uint8_t result = 0U;
    const vector_t *vector_p = (vector_t*)(app_abs_addr_start);

    if((vector_p->stack_addr& 0x2FF00000) == 0x20000000)
    {
        result = 1U;
