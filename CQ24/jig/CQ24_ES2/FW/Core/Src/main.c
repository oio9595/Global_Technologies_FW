...
void JigTaskMainTask(void const * argument)
{
  ...
  for(;;)
  {
		...
		{
			if(gn_cqic_test_type == 0)  /* function test */
			{
				test_procedure_run();	
			}
			else						  /* triming */
...
void TaskDebugUart(void const * argument)
{
  ...
  for(;;)
  {
    if(xQueueReceive( xQueue,&pMsgQ,( TickType_t )portMAX_DELAY ) == pdPASS)
    {
        ...
        else if(sscanf(str_in, "ui:din %d", &gb_NO_TRIM) == 1)
        {
            snprintf(msg_buffer, sizeof msg_buffer, "\r\n gb_NO_TRIM:%d\r\n ", gb_NO_TRIM);
            print(msg_buffer);
        }
        ...
        else if(sscanf(str_in, "ui:ctrim2 %d %f %d %d %f", &gui_GAIN_DAC_DAC_Input, &gf_GAIN_DAC_Step, &gui_GAIN_DAC_vHigh, &gui_GAIN_DAC_vLow, &gf_GAIN_DAC_Delta) == 5)
        {
        ...
        else if(sscanf(str_in, "ui:ctrim3 %d %f %d %f", &gui_DATA_DAC_DAC_Input, &gf_DATA_DAC_Step, &gui_DATA_DAC_Target_Offset_DAC, &gf_DATA_DAC_Voltage) == 4)
        {
        ...
        {
            cmd1.u.cmd_id	= CMD_01;
        	cmd1.u.addr 	= recv_reg_addr;
            cmd1.u.rw		= SPI_WR;
            cmd1.u.data 	= recv_reg_val;
            cqic_write(CQIC_1, &cmd1.value, 1);	/* set channel output enable */
            HAL_Delay(1);
            print("\n\rOK \0");
        }
        else if(sscanf(str_in, "cq_w %x %x", &recv_reg_addr, &recv_reg_val) == 2)
        {
            cmd2.u.cmd_id	= CMD_02;
        	cmd2.u.addr 	= recv_reg_addr;
            cmd2.u.rw		= SPI_WR;
            cmd2.u.data 	= recv_reg_val;
            cqic_write(CQIC_1, cmd2.value, 2);	/* set channel output enable */
            HAL_Delay(1);
            print("\n\rOK \0");
        }
        ...
        else if(sscanf(str_in, "cq_cs %d", &recv_param) == 1)
        {
            if (recv_param == 0)
            {
                CQIC1_NSCS_LO();
