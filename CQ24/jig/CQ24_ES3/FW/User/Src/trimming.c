...
extern void CQ24_Read(uint8_t device_id, uint16_t* cmd, uint16_t* pData, uint16_t length);
extern void CQ24_Write(uint8_t device_id, uint16_t* pData, uint16_t length);
...
void gt_cqic_test_init(void)
{
	...
	cmd2.u.data 	= 0xA5A5;
	CQ24_Write(CQIC_1, cmd2.value, 2);
...
void gt_get_dac10b_adc_data(uint8_t ch)
{
	...
	CQ24_Write(CQIC_1, cmd2.value, 2);	/* set frame header & DAC 10bit(gain) */
	HAL_Delay(10);
#endif
...
void cqic_12b_DAC_offset_trimming(uint8_t ch)
{
	...
	if(gub_off12v_loop_cnt[ch] > 30)
	{
		if(gub_off12v_saved_cnt[ch] > 0)
		{
			...
			CQ24_Write(CQIC_1, dac12b_offset.value, 2);
...
void cqic_12b_VREF_offset_trimming_12(uint16_t ch)
{
	...
	else
	{
		...
		CQ24_Write(CQIC_1, dac10b_offset.value, 2);
    ...
    if(gt_cqic_test_step != TEST_STEP_NONE)
    {
		switch(gt_cqic_test_step)
		{
			...
			case TRIMMING_STEP01_00:
				{
					...
					CQ24_Write(CQIC_1, cmd2.value, 2);	/* VREF 1.5V LDO INIT VAULE */
					HAL_Delay(30);
#else
				...
				if(gb_adc_start_dma == 0)
				{
					...
					if(gub_loop_cnt > 30)
					{
						if(gub_saved_cnt > 0)
						{
							...
							CQ24_Write(CQIC_1, cmd2.value, 2);	/* VREF 1.5V LDO offset VAULE */
							HAL_Delay(100);
				...
				if(gb_adc_drdy_flag2  == 1)
				{
					...
					else
					{
						...
						if(Vref_val <= 879)
						{
							...
							CQ24_Write(CQIC_1, cmd2.value, 2);	/* set ANA DAC 10bit */
							HAL_Delay(3);
  						...
  						if(dac12b_val <= gui_TRIM_RANGE_MAX)
						{
							...
							CQ24_Write(CQIC_1, cmd2.value, 2);	/* set Current DAC 12bit */
							HAL_Delay(3);
							gn_adc_read_count_adc1 = CONFIRM_COUNT;
							gn_adc_read_count_adc2 = 0;
							adc_ch = 0;
			...
			case TRIMMING_STEP05_00:
				...
				CQ24_Write(CQIC_1, &cmd1.value, 1);
