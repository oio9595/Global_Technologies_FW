...
typedef union tag_INTERRUPT_ENABLE
{
    ...
    struct
    {
        ...
        uint16_t int_ld_en                  :  1;   /* INT_LD_EN
                                                        ‘0’ No operation
                                                        ‘1’ This bit enables to issue the interrupt through the nINT_FAULT pin in case that the LD_DIFF_POINTER is equal to or smaller than the LD_DIFF_THRESOLD during the execution of the LD transfer to the XD-12.
                                                    */
...
/* ======================= ES1 ============================*/
#if 1 /* XC24_ES1 */
/* FB_PWM_PERIOD : default 0x00 */
typedef union tag_FB_PWM_PERIOD
{
    uint16_t ALL;
    struct
    {
        uint16_t fb_pwm_period                  : 11;   /* PB_PWM_PERIOD : This determines the period of PWM and counted in a MCLK. The 2KHz is recommended, but the value depends on the system condition for externa FBc ontrol circuit. */
        uint16_t                                :  2;   /* reserved */
        uint16_t t_fb_en                        :  1;   /* T_FB_EN : If this bit ‘1’, the FB signal toward the FB pin will be de-asserted as long as the thermal event is active, even though the FB event happens. */
        uint16_t fb_pwm_pol                     :  1;   /* FB_PWM_POL : This bit determines a polarity of the FB signal toward the external FBp in. If this bit ‘0’, the FB assertion will be showed in a low level on the FB pin because the pin has an open-drain property. */
        uint16_t fb_pwm_en                      :  1;   /* FB_PWM_EN : If this bit is set, the PWM signal determined by the FB_PWM_PERIOD will be driven on the external FB pin. If this bit is ‘0’, the signal, low or high, will be out to the external FB pin at the period of VSYNC or the FAULT_AUTO_READ_TIMER. */
    };
}_v_fb_pwm_period_t;

/* FB_PWM_DUTY : default 0x00 */
typedef union tag_FB_PWM_DUTY
{
    uint16_t ALL;
    struct
    {
        uint16_t fb_pwm_duty_high               :  8;   /* PB_PWM_DUTY_HIGH : This determines the active duration time of the FB_PWM_PERIOD. If the FB event occurs continuously, the active duration will be increased in an arithmetic way every FAULT_AUTO_READ command execution. */
        uint16_t fb_pwm_duty_wait               :  5;   /* FB_PWM_DUTY_WAIT : This determines how many periods of the FAULT_AUTO_READ execution the FB event does not occur, then reduces the active duration time in an arithmetic way. */
        uint16_t                                :  3;   /* reserved */
    };
}_v_fb_pwm_duty_t;

/* COMMAND_LATENCY : default 0x1F */
typedef union tag_COMMAND_LATENCY
{
    uint16_t ALL;
    struct
    {
        uint16_t cmd_latency                    :  8;   /* CMD_LATENCY : This determines the interval time between the end of a command and the start of a new command. The value can be increased in the read command execution if the XD12 mclk is so slower than the XC24 mclk, but it is recommended to keep the default value in most cases. */
        uint16_t                                :  8;   /* reserved */
    };
}_v_command_latency_t;

/* DAISIED DEVICE CHANNEL SIZE1 : default 0x00 */
typedef union tag_DAISIED_DEVICE_CHANNEL_SIZE1
{
    uint16_t ALL;
    struct
    {
        uint16_t daisied_dev_ch_size1   :  5;   /* DAISIED_DEV_CH_SIZE : This register should be set with a valid number for correct operating. The number means how many channels of the XD-12 drivers daisied in the relevant channel are activated. The maximum value of XD12 driver can be up to 12 channels. For future, these registers can support up to 31 channels in incoming XD driver. */
        uint16_t daisied_dev_ch_size2   :  5;
        uint16_t daisied_dev_ch_size3   :  5;
        uint16_t                        :  1;   /* reserved */
    };
}_v_daisied_device_channel_size1_t;

/* DAISIED DEVICE CHANNEL SIZE2 : default 0x00 */
typedef union tag_DAISIED_DEVICE_CHANNEL_SIZE2
{
    uint16_t ALL;
    struct
    {
        uint16_t daisied_dev_ch_size4   :  5;   /* DAISIED_DEV_CH_SIZE : This register should be set with a valid number for correct operating. The number means how many channels of the XD-12 drivers daisied in the relevant channel are activated. The maximum value of XD12 driver can be up to 12 channels. For future, these registers can support up to 31 channels in incoming XD driver. */
        uint16_t daisied_dev_ch_size5   :  5;
        uint16_t daisied_dev_ch_size6   :  5;
        uint16_t                        :  1;   /* reserved */
    };
}_v_daisied_device_channel_size2_t;

/* DAISIED DEVICE CHANNEL SIZE3 : default 0x00 */
typedef union tag_DAISIED_DEVICE_CHANNEL_SIZE3
{
    uint16_t ALL;
    struct
    {
        uint16_t daisied_dev_ch_size7   :  5;   /* DAISIED_DEV_CH_SIZE : This register should be set with a valid number for correct operating. The number means how many channels of the XD-12 drivers daisied in the relevant channel are activated. The maximum value of XD12 driver can be up to 12 channels. For future, these registers can support up to 31 channels in incoming XD driver. */
        uint16_t daisied_dev_ch_size8   :  5;
        uint16_t daisied_dev_ch_size9   :  5;
        uint16_t                        :  1;   /* reserved */
    };
}_v_daisied_device_channel_size3_t;

/* DAISIED DEVICE CHANNEL SIZE4 : default 0x00 */
typedef union tag_DAISIED_DEVICE_CHANNEL_SIZE4
{
    uint16_t ALL;
    struct
    {
        uint16_t daisied_dev_ch_size10  :  5;   /* DAISIED_DEV_CH_SIZE : This register should be set with a valid number for correct operating. The number means how many channels of the XD-12 drivers daisied in the relevant channel are activated. The maximum value of XD12 driver can be up to 12 channels. For future, these registers can support up to 31 channels in incoming XD driver. */
        uint16_t daisied_dev_ch_size11  :  5;
        uint16_t daisied_dev_ch_size12  :  5;
        uint16_t                        :  1;   /* reserved */
    };
}_v_daisied_device_channel_size4_t;

/* DAISIED DEVICE CHANNEL SIZE5 : default 0x00 */
typedef union tag_DAISIED_DEVICE_CHANNEL_SIZE5
{
    uint16_t ALL;
    struct
    {
        uint16_t daisied_dev_ch_size13  :  5;   /* DAISIED_DEV_CH_SIZE : This register should be set with a valid number for correct operating. The number means how many channels of the XD-12 drivers daisied in the relevant channel are activated. The maximum value of XD12 driver can be up to 12 channels. For future, these registers can support up to 31 channels in incoming XD driver. */
        uint16_t daisied_dev_ch_size14  :  5;
        uint16_t daisied_dev_ch_size15  :  5;
        uint16_t                        :  1;   /* reserved */
    };
}_v_daisied_device_channel_size5_t;

/* DAISIED DEVICE CHANNEL SIZE6 : default 0x00 */
typedef union tag_DAISIED_DEVICE_CHANNEL_SIZE6
{
    uint16_t ALL;
    struct
    {
        uint16_t daisied_dev_ch_size16  :  5;   /* DAISIED_DEV_CH_SIZE : This register should be set with a valid number for correct operating. The number means how many channels of the XD-12 drivers daisied in the relevant channel are activated. The maximum value of XD12 driver can be up to 12 channels. For future, these registers can support up to 31 channels in incoming XD driver. */
        uint16_t daisied_dev_ch_size17  :  5;
        uint16_t daisied_dev_ch_size18  :  5;
        uint16_t                        :  1;   /* reserved */
    };
}_v_daisied_device_channel_size6_t;

/* DAISIED DEVICE CHANNEL SIZE7 : default 0x00 */
typedef union tag_DAISIED_DEVICE_CHANNEL_SIZE7
{
    uint16_t ALL;
    struct
    {
        uint16_t daisied_dev_ch_size19  :  5;   /* DAISIED_DEV_CH_SIZE : This register should be set with a valid number for correct operating. The number means how many channels of the XD-12 drivers daisied in the relevant channel are activated. The maximum value of XD12 driver can be up to 12 channels. For future, these registers can support up to 31 channels in incoming XD driver. */
        uint16_t daisied_dev_ch_size20  :  5;
        uint16_t daisied_dev_ch_size21  :  5;
        uint16_t                        :  1;   /* reserved */
    };
}_v_daisied_device_channel_size7_t;

/* DAISIED DEVICE CHANNEL SIZE8 : default 0x00 */
typedef union tag_DAISIED_DEVICE_CHANNEL_SIZE8
{
    uint16_t ALL;
    struct
    {
        uint16_t daisied_dev_ch_size22  :  5;   /* DAISIED_DEV_CH_SIZE : This register should be set with a valid number for correct operating. The number means how many channels of the XD-12 drivers daisied in the relevant channel are activated. The maximum value of XD12 driver can be up to 12 channels. For future, these registers can support up to 31 channels in incoming XD driver. */
        uint16_t daisied_dev_ch_size23  :  5;
        uint16_t daisied_dev_ch_size24  :  5;
        uint16_t                        :  1;   /* reserved */
    };
...
typedef union _xc24_regs
{
    ...
    struct
    {
        ...
        _v_dummy_t                          _r44;
        _v_channe_enablel_t                 _r45;
        _v_channe_enable2_t                 _r46;
    };
}_xc24_regs_t;
