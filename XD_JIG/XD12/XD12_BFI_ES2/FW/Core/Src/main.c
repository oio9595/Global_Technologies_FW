...
/* ----------------- command list - xd ----------------- */
        ...
        else if (Command_is_("xd_trim_test"))
        {
            XDIC_Trim_Init_VREF_CTL();
            JigBD_IF_Start_MCU_ADC();
            uint16_t vref_adc =  JigBD_IF_Get_MCU_ADC();
            print(LOG_INFO, "\r\n VREF  : %.3f [V]", JigBD_IF_Convert_MCU_ADC_To_Volt(vref_adc));
            LL_mDelay(10);
