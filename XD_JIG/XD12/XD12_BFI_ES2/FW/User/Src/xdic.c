...
static void XDIC_Set_Delay_CH(void)
{
    if (gn_xd_ld_size == 0)
    {
        print(LOG_ERROR, "ERROR: %s - ld_size is zero(%d) !!\r\n", __func__, gn_xd_ld_size);
        while(1) {}
    }
    else
    {
...
{
    gt_xdic_general_regs._r0C.dev_max_curr_level = in_dev_max_curr;
    XDIC_Write_General_Reg(XDIC_ADDR_FAULT_LEVEL, gt_xdic_general_regs._r0C.val);
}
...
static void XDIC_Set_OSC_Compensation(uint8_t osc_comp)
{
    gt_xdic_general_regs._r2B.osc_comp = osc_comp;
    XDIC_Write_General_Reg(XDIC_ADDR_OSC_COMPENSATION, gt_xdic_general_regs._r2B.val);
}
...
void XDIC_Trim_Init_OSC(void)
{
    ...
    XDIC_Set_OSC_Compensation(XDIC_OSC_TRIM_OSC_COMP);
}
